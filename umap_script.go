...
func optimize(embedding [][]float64,edges []UMAPEdge,nEpochs int,learningRate float64,negativeSamples int,minDist float64) {
	...
	if nEpochs <= 0 {nEpochs = 200}
	if learningRate <= 0 {learningRate = 1.0}
	if negativeSamples < 0 {negativeSamples = 0}
	if minDist <= 0 {minDist = 0.1}
	minDist2 := minDist * minDist

	for epoch := 0; epoch < nEpochs; epoch++ {
		// shuffle edges each epoch for SGD
		...
		for _, e := range edges {
			...
			if dist2 > 0 {
				gradCoeff := learningRate * e.Weight / (0.001 + dist2)
			...
			// sample random points and push them further. 
			if negativeSamples == 0 {
				continue
			}
			...
			for s := 0; s < negativeSamples; s++ {
				...
				// Repulsion: stronger when k is too close to i.
				gradCoeffNeg := learningRate * e.Weight * minDist2 / (0.001 + dist2Neg)

				for d := 0; d < dim; d++ {
					diff := yi[d] - yk[d]
					grad := gradCoeffNeg * diff
					// push k away from i (and i away from k)
					yi[d] += grad
					yk[d] -= grad
				}
			}
